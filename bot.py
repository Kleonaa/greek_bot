...
def looks_like_greek_verb(greek_word: str):
    return greek_word.endswith(("ω", "μαι"))


async def generate_verb_forms(greek_word: str, translation: str):
    if not OPENAI_API_KEY or not looks_like_greek_verb(greek_word):
        return None, None
    try:
        from openai import AsyncOpenAI
        client = AsyncOpenAI(api_key=OPENAI_API_KEY)
        resp = await client.chat.completions.create(
            ...
            messages=[{
                "role": "user",
                "content": (
                    f"The Greek word is '{greek_word}' (Russian meaning: {translation}). "
                    "If it is not a Greek verb in present-tense dictionary form, reply exactly: NOT_VERB. "
                    "If it is a verb, give the most common Modern Greek simple future form and simple past form. "
                    "Use first-person singular forms, matching this style: θα κάνω | έκανα. "
                    "Format exactly: <future> | <past>. No extra text."
                ),
            }],
            max_tokens=60,
        )
        ...
        logging.warning(f"OpenAI verb forms error: {e}")
    return None, None
...
async def get_word_verb_forms_text(word: dict):
    greek_word = word["greek"]
    forms = db.get_verb_forms_by_present(greek_word)
    if not forms and OPENAI_API_KEY and looks_like_greek_verb(greek_word):
        future, past = await generate_verb_forms(greek_word, word["translation"])
        if future and past:
            db.save_verb_form(
                greek_word, future, past, word["translation"], "generated by AI"
            )
            forms = (future, past)
...
async def show_word_answer(query, word: dict, idx: int, total: int, context: ContextTypes.DEFAULT_TYPE):
    verb_forms_text = await get_word_verb_forms_text(word)
